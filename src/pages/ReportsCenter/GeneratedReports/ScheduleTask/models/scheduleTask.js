...
export default {
  ...
  reducers: {
    ...
    saveScheduledTaskDetail(state, { payload }) {
      ...
      doFilter(filterList).forEach(
        ({ filterName: label, filterValue, keyMap, type, filterType, isRequiredWhere }) => {
          let texts = filterValue && filterValue.split(',');
          // RANGE_PICKER special deal
          if (
            ['Daily', 'Monthly'].includes(cronType) &&
            filterType === 'RANGE_PICKER' &&
            isRequiredWhere === '1'
          ) {
            const reportNameSplit = taskDetail.taskName.split('_')[1].split('-');
            const text = `${moment(reportNameSplit[0], 'YYYYMMDD').format(
              'DD-MMM-YYYY'
            )} ~ ${moment(reportNameSplit[1], 'YYYYMMDD').format('DD-MMM-YYYY')}`;
            texts = [text];
          } else if (keyMap) {
            if (texts) {
