...
export default {
  ...
  effects: {
    *fetch({ payload = {} }, { call, put, select }) {
      const { filter, pagination, cronTypeList, isExecute } = yield select(state => state.adhoc);
      ...
      if (resultCode === '0' || (resultCode === 0 && result)) {
        const {
    ...
    *reportTypeSelect(_, { call, put, select }) {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname !== '/ReportsCenter/GeneratedReports/AdHoc') {
          dispatch({ type: 'clear' });
        }
      });
    },
