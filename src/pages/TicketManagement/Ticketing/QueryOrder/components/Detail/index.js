...
class Detail extends React.Component {
  ...
  showInformation = (detailType, detailList, vidResultList, patronInfo, netAmt, refundSuccessFlag, pageLoading, userType, revalidationVidListVisible, revalidationVidList, revalidationVidLoading) => {
        ...
        <Spin spinning={!!pageLoading}>
          <Form className={styles.formStyle}>
            ...
            >
              <Tooltip
                placement="topLeft"
                title={<span style={{ whiteSpace: 'pre-wrap' }}>{`$ ${String(Number(netAmt).toFixed(2)).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`}</span>}
              >
                <span className={styles.drawerTitleStyle}>{`$ ${String(Number(netAmt).toFixed(2)).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`}</span>
              </Tooltip>
            </FormItem>
            }
  ...
  render() {
    ...
    return (
      <Drawer
        ...
        maskClosable={false}
      >
        {this.showInformation(detailType, detailList, vidResultList, patronInfo, netAmt, refundSuccessFlag, pageLoading, userType, revalidationVidListVisible, revalidationVidList, revalidationVidLoading)}
      </Drawer>
    );
  }
}
